...
void InitPolyBook() {
    ...
    FILE *pFile = fopen("books/performance.bin","rb");
    ...
    if(pFile == NULL) {
        ...
        entries = (POLY_BOOK_ENTRY*)malloc(NumEntries * sizeof(POLY_BOOK_ENTRY));
        rewind(pFile);
        
        size_t returnValue;
        returnValue = fread(entries, sizeof(POLY_BOOK_ENTRY), NumEntries, pFile);
